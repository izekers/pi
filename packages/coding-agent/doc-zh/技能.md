> pi 可以创建技能。告诉它为你的使用场景构建一个。

# 技能

技能是自主的能力包，agent 按需加载。技能为特定任务提供专门的工作流程、设置说明、辅助脚本和参考文档。

Pi 实现了 [Agent Skills 标准](https://agentskills.io/specification)，对大多数违规行为发出警告但保持宽容。Pi 允许技能名称与其父目录不同，尽管标准不允许这样做；该规则对于跨多个 agent工具使用的共享技能目录来说是次优的。
...
> **安全注意：** 技能可以指示模型执行任何操作，并且可能包含模型调用的可执行代码。使用前请审查技能内容。
...
- 项目（仅在项目被信任后）：
  - `.pi/skills/`
  - `.agents/skills/` 在 cwd 和祖先目录中（直到 git仓库根，或不在仓库时到文件系统根）
- 包：`package.json` 中的 `skills/` 目录或 `pi.skills` 条目
- 设置：`themes` 数组包含文件或目录
- CLI：`--skill <path>`（可重复，即使使用 `--no-skills` 也可添加）

发现规则：
- 在 `~/.pi/agent/skills/` 和 `.pi/skills/` 中，直接的根 `.md` 文件被发现为单独技能
...
2. 系统提示以 [规范](https://agentskills.io/integrate-skills) 规定的 XML 格式包含可用技能
3. 当任务匹配时，agent 使用 `read` 加载完整的 SKILL.md（模型并不总是这样做；使用提示或 `/skill:name` 强制加载）
4. agent 按照说明操作，使用相对路径引用脚本和资产
...
技能是一个带有 `SKILL.md` 文件的目录。其他都是自由形式。
...
根据 [Agent Skills规范](https://agentskills.io/specification#frontmatter-required)：

| 字段 | 必需 | 说明 |
|-------|----------|-------------|
| `name` | 是 |最多 64 字符。小写 a-z、0-9、连字符。与标准不同，Pi 不要求此名称匹配父目录，因为该标准要求对于跨多个 agent 工具使用的共享技能目录来说是次优的。 |
| `description` | 是 | 最多 1024 字符。技能的作用及何时使用。 |
...
- 不允许前导/尾随连字符
- 不允许连续连字符
Pi 不要求名称匹配父目录。Agent Skills 标准要求这样做，但对于跨多个工具使用的共享技能目录来说，该要求是次优的。
...
名称冲突（不同位置的相同名称）发出警告并保留找到的第一个技能。
